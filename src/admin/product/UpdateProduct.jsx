...
const UpdateProduct = () => {
    ...
    const handleNext = async () => {
        ...
        if (currentStep === 1) {
            ...
            // For update, pic is not required since we already have images
            isValid = await trigger(['color', 'size'])
    ...
    useEffect(() => {
        if (product) {
            setValue('product._id', product._id)
            ...
            setValue('stock', product.stock ? 'true' : 'false')
            setValue('stockQuantity', product.stockQuantity)
            setValue('description', product.description)
            setValue('active', product.active ? 'true' : 'false')
        }
    }, [product, setValue])

    const formSubmit = (data) => {
        // Handle images properly for update
        let images = []
        if (data.pic && data.pic.length > 0) {
            // Check if files are selected (new images)
            const fileList = Array.from(data.pic)
            images = fileList.filter(file => file instanceof File)
        }

        // Remove the pic field from data to avoid serialization issues
        const { pic, ...formData } = data

        dispatch(updateProductRequest({ _id, ...formData, images }))
